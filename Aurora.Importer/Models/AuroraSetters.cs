...
public class AuroraSetters
{
    ...
    public string? names_format { get; set; }
    public string? hd { get; set; }
    public string? hit_die { get => hd; set => hd = value; }
    ...
    public IEnumerable<AuroraSetterEntry> FindEntries(string setterName) =>
        entries.Where(x => string.Equals(x.name, setterName, StringComparison.OrdinalIgnoreCase));

    public AuroraSetterEntry? FindEntry(string setterName) => FindEntries(setterName).FirstOrDefault();

    public string? GetValue(string setterName) => FindEntry(setterName)?.value;
