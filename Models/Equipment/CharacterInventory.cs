...
public class CharacterInventory : ObservableObject
{
  ...
  public void EquipArmor(RefactoredEquipmentItem equipment)
  {
    ...
    this.CalculateAttunedItemCount();
    ApplicationManager.Current.EventAggregator.Send<ReprocessCharacterEvent>(new ReprocessCharacterEvent());
  }
  ...
  {
    if (!ApplicationManager.Current.Settings.Settings.RequestAddAttackWhenEquippingWeapon || !CharacterManager.Current.Status.IsLoaded || !this.Character.AttacksSection.Items.All<AttackSectionItem>((Func<AttackSectionItem, bool>) (x => x.EquipmentItem != equipment)))
      return;
