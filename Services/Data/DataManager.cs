...
public sealed class DataManager
{
  ...
  private DataManager() => this._eventAggregator = ApplicationManager.Current.EventAggregator;
  ...
  public async Task<IEnumerable<ElementBase>> InitializeElementDataAsync()
  {
    ...
    if (exceptions.Any<Exception>())
    {
      ...
      if (exceptions.Any<Exception>())
      {
        ...
        MessageDialogService.ShowException(ex, "Error(s) parsing data files", exceptions.Count > 1 ? $"{exceptions.Count} exceptions occurred while parsing the data files. The first one is shown below, the others can be found in the logs.\r\nFile: {obj1}\r\nInfo: {obj2}" : $"An exception occurred while parsing the data files.\r\nFile: {obj1}\r\nInfo: {obj2}");
      }
    }
