  ...
  ISubscriber<CharacterManagerElementUnregistered>
{
    ...
    foreach (SelectionElement selectionElement in selectionElementList2)
    {
      if (!selectionElement.IsDefault)
      {
        if (selectionElement.IsChosen)
          ApplicationManager.Current.SendStatusMessage($"'{selectionElement.Element.Name}' is prepared but is restricted by source. Prepare another spell.");
        else
  ...
  private void TogglePrepareSpell()
  {
    ...
    if (this.SelectedKnownSpell.Element.AsElement<Spell>().Level == 0)
    {
      ...
      MessageDialogService.Show(this.SelectedKnownSpell.DisplayName + " (cantrip) is always ready to cast and doesn't need to be prepared.");
    }
    else
    {
      if (this.SelectedKnownSpell.IsChosen)
      {
        ...
        {
          MessageDialogService.Show(this.SelectedKnownSpell.DisplayName + " is always prepared for you.");
          return;
        }
  ...
  private void Prepare()
  {
    ...
    {
      MessageDialogService.Show("This spell is always prepared and was granted by " + this.SelectedKnownSpell.Element.Aquisition.GrantRule.ElementHeader.Name);
    }
    else
    {
      if (((IEnumerable<ElementBase>) this.PreparedSpells).Contains<ElementBase>(this.SelectedKnownSpell.Element))
