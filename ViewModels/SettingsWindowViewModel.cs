...
public sealed class SettingsWindowViewModel : ViewModelBase
{
  ...
  {
    this.Version = Resources.ApplicationVersion;
    this.AssemblyVersion = Assembly.GetExecutingAssembly().GetName().Version.ToString();
    this.StyleSheet = Builder.Presentation.Properties.Settings.Default.Theme.Contains("Dark") ? DataManager.Current.GetResourceWebDocument("stylesheet-dark.css") : DataManager.Current.GetResourceWebDocument("stylesheet.css");
  ...
  private void BrowseCustomRootDirectory()
  {
    ...
    {
      SelectedPath = this.CustomRootDirectory
    };
    if (folderBrowserDialog.ShowDialog() != DialogResult.OK)
      return;
    this.CustomRootDirectory = folderBrowserDialog.SelectedPath;
  }

  private void BrowseAdditionalCustomDirectory()
  {
    string additionalCustomDirectory = this.AdditionalCustomDirectory;
    FolderBrowserDialog folderBrowserDialog = new FolderBrowserDialog()
    {
      SelectedPath = this.AdditionalCustomDirectory
    };
    if (folderBrowserDialog.ShowDialog() != DialogResult.OK)
      return;
    this.AdditionalCustomDirectory = folderBrowserDialog.SelectedPath;
  }
