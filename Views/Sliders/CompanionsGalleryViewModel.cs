...
// Type: Builder.Presentation.Views.Sliders.CompanionsGalleryViewModel
...
public class CompanionsGalleryViewModel : ViewModelBase
{
  ...
  public CompanionsGalleryViewModel()
  {
    if (this.IsInDesignMode)
    {
      foreach (string companionFilename in DesignData.CompanionFilenames)
        this.Portraits.Add(companionFilename);
    ...
    {
      this.GetFiles();
      FileSystemWatcher fileSystemWatcher = new FileSystemWatcher(DataManager.Current.UserDocumentsCompanionGalleryDirectory);
  ...
  {
    this._files = ((IEnumerable<string>) Directory.GetFiles(Path.Combine(DataManager.Current.UserDocumentsCompanionGalleryDirectory))).Where<string>((Func<string, bool>) (x => x.ToLower().EndsWith(".jpg") || x.ToLower().EndsWith(".jpeg") || x.ToLower().EndsWith(".png"))).ToList<string>();
  }
